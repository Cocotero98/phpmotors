...
// This function checks for an existing email adress
function checkExistingEmail($emailSubmitted){
    ...
    $stmt->bindvalue(':clientEmail', $emailSubmitted, PDO::PARAM_STR);
...
function updateAccount($clientFirstname,$clientLastname,$clientEmail,$clientId){
    ...
    $stmt->execute();
    $rowsChanged = $stmt->rowCount();
    $test = "UPDATE clients SET clientFirstname = $clientFirstname, clientLastname = $clientLastname, 
    clientEmail = $clientEmail WHERE clientId = $clientId";
    $stmt->closeCursor(); 
    return $rowsChanged;
}
